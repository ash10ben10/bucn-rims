...
<head>
	...
	<?php include "eqpspec_engine.php"; ?>
		...
		$("#iname").on("change", function(){
			...
			$.ajax({
					...
					$.each(response.data, function(index, element){
						...
						$itUnit.append("<option value='"+element.untid+"'>"+element.untname+"</option>");
					});
					$itUnit.selectpicker("refresh");
					$itUnit.trigger("change");
				}).fail(function(){
					...
					var hr = new XMLHttpRequest();
						...
						if(hr.readyState == 4 && hr.status == 200) {
						  var return_data = hr.responseText;
							...
							window.location = "equipment_specs.php";
	...
	function submitform(e){
		...
		var $iname = $("#iname").val();
		if($iname == 0 || $iname == "" || $iname == null){ 
			isOk = false;
			msg;
		}
		
		var $iunit = $("#iunit").val();
		if($iunit == 0 || $iunit == "" || $iunit == null){ 
			isOk = false;
			msg;
		}
	...
	<div id="content-wrapper">
		...
		<div class="container-fluid">
			...
			<div class="row" style="margin-top:-20px;">
				<div class="col-lg-12">
					<h1 style="font-family: Calibri;">&nbsp;<i class="zmdi zmdi-widgets zmdi-hc-lg"></i>&nbsp;&nbsp;Equipment Descriptions</h1>
							...
							<div class="row">
								...
								<div class="col-lg-12">
									<div class="col-lg-3" style="margin-bottom: 10px;" align="left">
										...
										<select name="iname" id="iname" class="selectpicker form-control" data-hide-disabled="true" data-live-search="true" required />
											<?php 
												$query = mysql_query("SELECT * FROM `items` WHERE item_type='Equipment'");
									...
									<div class="col-lg-2">
										...
										<select class="selectpicker form-control" name="iunit" id="iunit" value="" data-hide-disabled="true" data-live-search="true" onChange="" required >
										</select>
									</div>
									<div class="col-lg-3">
										<label>Description:</label>
										<input class="form-control" name="idesc" id="idesc" placeholder="Item Description" required />
									</div>
									<div class="col-lg-2">
										<label>Price:</label>
										<input class="form-control" name="iprice" id="iprice" placeholder="Item Price:" pattern="([0-9])+" min="0.01" type="number" step="0.01" required />
									</div>
						...
						$getdesc = mysql_query("SELECT md.md_id, i.item_name, iu.item_unit_name, md.description, md.price FROM `more_desc`AS md LEFT JOIN more_units AS mu ON mu.munit_id = md.munit_id LEFT JOIN items AS i ON i.item_id = mu.item_id LEFT JOIN item_unit AS iu ON iu.item_unit_id = mu.item_unit_id WHERE i.item_type = 'Equipment'");
						
						if(mysql_num_rows($getdesc) == 0){
							print "<br /><p align=center><i>There are no available Descriptions on the Equipment. Please add one.</i></p><br />";
						}else{
