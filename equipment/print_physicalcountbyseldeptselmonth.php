...
$mon = $mont->format("F");
$deptname = mysql_fetch_array(mysql_query("SELECT `dept_name` FROM `department` WHERE dept_id = '$dept'"));
...
$personnel = mysql_fetch_array(mysql_query("SELECT ".$selctP." FROM personnel WHERE personnel_id = $selectpersonnel_id[personnel_id]"))or die (mysql_error());
...
$geteqps = mysql_query("SELECT * FROM `equipments` WHERE YEAR(eqpdate) = '$year' AND MONTH(eqpdate) = '$month' AND dept_id = '$dept' ORDER BY date_acquired DESC");
	...
	$pdf->Cell(335,8,"As at ".$mon." ".$year." at ".$deptname['dept_name'],'LR', 1, 'C');
	$pdf->SetFont('Arial','',10);
	$pdf->Cell(335,8,"for which ".$personnel['full_name'].", Supply Officer of BUCN is accountable, having assumed such accountability on ".$mon." ".$year.".",'LR', 1, 'C');
