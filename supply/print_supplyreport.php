...
$pdf->SetFont('Arial','B',14);
	...
	$pdf->Cell(335,8,'MONTHLY REPORT OF  STOCK AND ISSUED INVENTORY SUPPLIES','LR', 1, 'C');
		$pdf->SetFont('Arial','i',8);
	$mont = DateTime::createFromFormat("m", $month);
	$mon = $mont->format("F");
	$pdf->Cell(335,8,"As of ".$mon." ".$year,'LRB', 1, 'C');
	...
	while($retdata = mysql_fetch_array($getdates)){
		...
		$pdf->Cell(30,8,date("d", strtotime($retdata['recdate'])),'LB', 0, 'C');
