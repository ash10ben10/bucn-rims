...
<body>
	...
	<div id="content-wrapper">
		...
		<div class="container-fluid">
			...
			<div class="row" style="margin-top:-20px;">
				...
				<div class="col-lg-12">
					<h1 style="font-family: Calibri;">&nbsp;<i class="zmdi zmdi-file zmdi-hc-lg"></i>&nbsp;&nbsp;Property Acknowledgement Receipt</h1>
					<br />
					<form method="POST" id="addpar" enctype="multipart/form-data">
						<div class="panel panel-default" style="box-shadow: 0 4px 8px 0 rgba(0, 0, 0, 0.2), 0 6px 20px 0 rgba(0, 0, 0, 0.19);">
									...
									<div class="panel-body">
										...
										<div class="col-lg-6" style="margin-top:15px;">
											...
											<div class="form-group col-lg-12" style="font-size:15px;">
												...
												<input class="form-control" id="eqpbrand" name="eqpbrand" required />
											</div>
											<div class="form-group col-lg-12" style="font-size:15px;">
												...
												<div class="table-responsive">
													<table class="table table-striped table-bordered table-hover">
														...
														for($row = 1; $row<=$getitemdetails['quantity']; $row++){
															?>
															<tr>
																<!--<td rowspan="3" style="text-align:center;vertical-align:middle;"><?php //print $getstockitems['quantity']; ?></td>-->
																...
																<!--<td rowspan="3" style="text-align:center;vertical-align:middle;">
																	<?php //print "Php ".number_format($getitemdetails['est_unit_cost'] * ($getstockitems['quantity']), 2,'.',',');?>
																</td>-->
																<!--<td style="text-align:center;vertical-align:middle;">
																	<textarea class="form-control" style="resize:none;" id="propnum" name="propnum" placeholder="Property Number:" required></textarea>
																</td>-->
															</tr>
															<?php
														}
							...
							<div class="panel-footer" align="right">
								<button type="submit" name="parsave" id="parsave" class="btn btn-success"><span class='glyphicon glyphicon-floppy-disk'></span>&nbsp;Submit</button>
	...
	if(isset($_POST['parsave'])){
		...
		$genPARSql = "SELECT pardate,".
			" CONCAT('".$month."-',(COUNT(DATE_FORMAT(pardate, '%Y-%m')) + 1)) AS parnum".
			" FROM eqp_par".
			" GROUP BY DATE_FORMAT(pardate, '%Y-%m')".
			" HAVING DATE_FORMAT(pardate, '%Y-%m') = '".$month."'".
			" ORDER BY pardate DESC LIMIT 1";
		$genPARQry = mysql_query($genPARSql) ;
		if(mysql_num_rows($genPARQry) == 0){
			$parnum = "PAR-".$month."-1";
		}else{
			$genPARArr = mysql_fetch_array($genPARQry);
			$parnum = "PAR-".$genPARArr['parnum'];
		}
		
		mysql_query("LOCK TABLE eqp_par WRITE;");
				
		try{
			mysql_query("INSERT INTO `eqp_par`(
			`pardate`,
			`parnum`,
			`date_acquired`,
			`total_cost`,
			...
			print "<script>alert('Something went wrong when saving PAR to the System. Please check your connection.')</script>";
		}
		mysql_query("UNLOCK TABLE;");
		
		#get par_id
		$getparid = mysql_fetch_array(mysql_query("SELECT par_id FROM eqp_par WHERE par_id IN (SELECT MAX(par_id) FROM eqp_par) ")); //this makes the select last id recorded.
		$parid = $getparid[0];
		...
		$updateRI = "UPDATE `request_items` SET `icspar` = 'PAR', `icspar_id` = '$parid', issuance = 'Ready' WHERE `req_item_id` = '$readreqitem_id' ";
		mysql_query($updateRI);

		print "<script>alert('Property Acknowledgement Receipt has been saved successfully.'); window.location='add_ris.php?id=".$getitemdetails['pr_id']."';</script>";
	}
